...
class PrismaHighlighter {
    ...
    highlight(schema) {
        return schema.replace(/\r\n/g, "\n").split("\n").map(_line => {
            const line = _line.replace(/@\w+(?=\s|$)/g, match => this.colorize(match, "greenBright")).replace(/(@\w+)(\()([^()]*)(\))/g, (_, annotation, openBracket, args, closeBracket) => {
                const content = args.split(',').map((arg) => {
                    if (/^\s*"([^"]+)"\s*$/.test(arg))
                        return this.colorize(arg, "purpleDark");
                    if (arg.includes(":")) {
                        const [key, value] = arg.split(":").map(part => part.trim());
                        const coloredValue = value.replace(/\[(.*?)\]/g, (_, inner) => `${this.colorize("[", "yellowBright")}${this.colorize(inner, "orange")}${this.colorize("]", "yellowBright")}`);
            ...
            }).replace(/\b(model|enum)\s+([A-Z][a-zA-Z0-9_]*)/g, (_, keyword, modelName) => `${this.colorize(keyword, "cyanBold")} ${this.colorize(modelName, "purple")}`).replace(/\/\/(.*)$/gm, (_, comment) => this.colorize("// " + comment, "green")).replace(/\/\*([\s\S]*?)\*\//g, (_, content) => this.colorize(`/*${content}*/`, "greenDark")).replace(/^(\s*\S+)(.*)$/, (match, firstPart, secondPart) => {
...
exports.PrismaHighlighter = PrismaHighlighter;
